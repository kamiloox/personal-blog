...
category: javascript
published: true
excerpt: Przechowywanie tokena na frontendzie to jeden z najcięższych tematów do zrozumienia dla początkujących developerów. Ale warto dobrze zrozumieć ten temat, ponieważ nie możemy pozwolić komuś na kradzież tożsamości naszych użytkowników 🔐
---
...
Przechowywanie danych w localStorage jest jakimś rozwiązaniem, ale trzeba pamiętać, że to zostaje zapisane w przeglądarce użytkownika, a jak wiadomo __wszystko co znajduje się na frontendzie może zostać przez każdego sprawdzone, nawet przez osobe trzecią__! Taki sposób jest podatny na ataki __[XSS](https://sekurak.pl/czym-jest-xss/)__ i ktoś może _wstrzyknąć_ wywołanie skryptu na naszej stronie, który następnie przechwyci token. Przecież nie chcemy, żeby trafił on w nieodpowiednie ręce.
...
Flaga _sameSite_ wprowadza restrykcje z jakich domen mogą wychodzić autoryzowane zapytania. Zabezpiecza nas przed stronami, które wykonują zapytania do naszej strony wraz z podpiętym tokenem. Czyli nikt z nieupoważnionej strony nie będzie mógł pobierać zasobów do których nie ma dostępu, nawet gdy taki ktoś ma podpięty token w ciasteczku. Ten rodzaj ataku ma nazwę [CSRF](https://pl.wikipedia.org/wiki/Cross-site_request_forgery). 
...
Już teraz nasza aplikacja jest o wiele bezpieczniejsza niż to co mieliśmy na początku. Teraz tylko rola serwera backendowego, żeby sprawdzał przychodzące do niego zapytania. Bardzo dobrze do tego nadaje się _middleware_, poniważ weryfikuje czy token jest nieprawidłowy, lub czy go nie ma wcale, a jeżeli wszystko jest poprawne, to zwraca żądany zasób.
...
const authMiddleware = (req, res, next) => {
  ...
  try {
    ...
    return res.status(403).json({ status: 403, message: 'Nieprawidłowy token' });
  }
};
```
