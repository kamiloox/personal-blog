...
Gdy ustawimy opcję _credentials_ na _same-origin_, to ciasteczka będą dołączone do zapytania, ale tylko w obrębie tego samego _origin_. Czyli nasz token, który znajduje się w ciasteczku, będzie przesyłany za każdym razem do serwera. Natomiast __[origin](https://developer.mozilla.org/en-US/docs/Glossary/Origin)__ jest tym co widzimy w naszym pasku adresu przeglądarki.
...
Jako _sameSite_ można ustawić wartość _secure_. Jednak ta opcja jest zbyt restrykcyjna i blokuje linki, które są na innych stronach i kierują do naszego serwisu. Dlatego istnieje również opcja _Lax_. Jest to złoty środek, ponieważ blokuje odbieranie ciasteczek z innych domen, ale umożliwia robić przekierowania do naszej strony.
...
Już teraz nasza aplikacja jest o wiele bezpieczniejsza niż to co mieliśmy na początku. Teraz tylko rola serwera backendowego, żeby sprawdzał przychodzące do niego zapytania. Bardzo dobrze do tego nadaje się _middleware_. Sprawdza czy token jest nieprawidłowy, lub czy go nie ma wcale. Jeżeli jednak wszystko jest poprawne, to zwraca żądany zasób.
